...
#' @examples
#' @author Beth Signal
removeExonInTranscript <- function(exonRanges, equal_exons, gtf.exons, glueExons=TRUE, replaceVariableExons=TRUE){
...
#' Replace a skipped exon into the transcripts it is skipped in
#' @param exonRanges GRanges object with ranges for exons
#' @param equal_exons data.frame generataed by findExonContainingTranscripts()
#' @param gtf.exons GRanges object made from a GTF with ONLY exon annotations (no gene, transcript, CDS etc.)
#' @param glueExons Join together exons that are not seperated by exons?
...
#' @examples
#' @author Beth Signal
replaceExonInTranscript <- function(exonRanges, equal_exons, skippedExonTranscripts, glueExons=TRUE, replaceVariableExons=TRUE){
    ...
    m <- match(skippedExonTranscripts$transcript_id, exonRanges$new_transcript_id)
    skippedExonTranscripts$new_transcript_id <-  skippedExonTranscripts$transcript_id
    skippedExonTranscripts$transcript_id <-  exonRanges$transcript_id[m]

    mcols(skippedExonTranscripts) <- mcols(skippedExonTranscripts)[,c('gene_id','transcript_id','transcript_type','exon_id','exon_number', 'new_transcript_id','overlaps')]
    mcols(exonRanges) <- mcols(exonRanges)[,c('gene_id','transcript_id','transcript_type','exon_id','exon_number','new_transcript_id','overlaps')]

    gtf_transcripts <- c(skippedExonTranscripts, exonRanges)
    ...
    gtf_transcripts$exon_number <- as.numeric(gtf_transcripts$exon_number)

    gtf_transcripts <- reorderExonNumbers(gtf_transcripts)
    order <- order(gtf_transcripts$transcript_id, gtf_transcripts$exon_number)
    gtf_transcripts <- gtf_transcripts[order]
    ...
    if(glueExons==TRUE){
        ...
        # split pos/neg ordering
        gtf_transcripts.neg <-
            gtf_transcripts[which(as.logical(strand(gtf_transcripts) == "-"))]
        order <- order(gtf_transcripts.neg$transcript_id,
                       plyr::desc(gtf_transcripts.neg$exon_number))
        gtf_transcripts.neg <- gtf_transcripts.neg[order]

        gtf_transcripts.pos <-
            gtf_transcripts[which(as.logical(strand(gtf_transcripts) == "+"))]
        gtf_transcripts <- c(gtf_transcripts.pos,gtf_transcripts.neg)

        #extend starts <---<---<---
        w <- which(end(ranges(gtf_transcripts))[-length(gtf_transcripts)] == start(ranges(gtf_transcripts[-1])))
        gtf_transcripts <- gtf_transcripts
        GenomicRanges::start(GenomicRanges::ranges(gtf_transcripts))[w+1] <-
            GenomicRanges::start(GenomicRanges::ranges(gtf_transcripts))[w]

        # remove exons that cover now redundant regions
        overlaps <- findOverlaps(gtf_transcripts, type="within")
        overlaps <- overlaps[overlaps@from == overlaps@to - 1]
        rm <- unique(overlaps@from)
        if(length(rm) >0){
            gtf_transcripts <- gtf_transcripts[-rm]
        }

        #extend ends --->--->--->
        overlaps <- findOverlaps(gtf_transcripts)
        overlaps <- overlaps[overlaps@from==overlaps@to +1]

        GenomicRanges::end(GenomicRanges::ranges(gtf_transcripts))[overlaps@to] <-
            GenomicRanges::end(GenomicRanges::ranges(gtf_transcripts))[overlaps@from]
...
#' @examples
#' @author Beth Signal
reorderExonNumbers <- function(gtf.exons){
    order <- order(gtf.exons$transcript_id, start(gtf.exons))

    gtf.exons <- gtf.exons[order]

    transcript_table <- as.data.frame(table(gtf.exons$transcript_id))
    transcript_table$strand <- as.character(strand(gtf.exons[match(transcript_table$Var1, gtf.exons$transcript_id)]))
