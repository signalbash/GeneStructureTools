...
#' @param allORFs orf information for all transcripts for novel sequence comparisons. Generated by getOrfs()
...
#' orfsProteinCoding <- getOrfs(gtf.exons[gtf.exons$gene_name=="Prex2" & gtf.exons$transcript_type=="protein_coding"], BSgenome = g)
#' orfsNMD <- getOrfs(gtf.exons[gtf.exons$gene_name=="Prex2" & gtf.exons$transcript_type=="nonsense_mediated_decay"], BSgenome = g)
...
#' orfsSkipped <- getOrfs(ExonSkippingTranscripts[ExonSkippingTranscripts$set=="skipped_exon"],BSgenome = g)
#' orfsIncluded <- getOrfs(ExonSkippingTranscripts[ExonSkippingTranscripts$set=="included_exon"],BSgenome = g)
...
orfDiff <- function(orfsX,
    ...
    orfChanges$max_percent_orf_shared <- (maxLength - abs(orfChanges$orf_length_bygroup_x - orfChanges$orf_length_bygroup_y)) /
        maxLength
    ...
    if(geneSimilarity == TRUE & !is.null(allORFs)){
        ...
        geneMatches <- lapply(orfChanges$gene_id, function(x) which(!is.na(match(allORFs$gene_id, x))))
        geneMatches <- unlist(geneMatches)

        idMatches <- mapply(function(x,y) rep(x,length(y)) ,(1:length(orfChanges$gene_id)), geneMatches)

        #geneMatches <- lapply(orfChanges$gene_id, function(x) which(!is.na(match(orfsX$gene_id, x))))
        #idMatches <- mapply(function(x,y) rep(x,length(y)) ,(1:length(orfChanges$gene_id)), geneMatches)
...
attrChangeAltSpliced <- function(orfsX,
    ...
    if(nrow(orfsX) > 0 & nrow(orfsY) > 0){
        ...
        if(compareUTR == TRUE & attribute == "orf_length"){
            if(all(grepl("AS", orfsX$id))){
                id.x <- paste0(attributeComparisons$id,"_" ,attributeComparisons$orf_length_bygroup_x)
                if(compareBy == "gene"){
                    m1 <- match(id.x, paste0(unlist(lapply(str_split(orfsX$id, "AS "), "[[", 2)), "_", orfsX$orf_length))
                }else{
                    m1 <- match(id.x, paste0(orfsX$id, "_", orfsX$orf_length))
                }
            }else{
                matchToGene <- match(attributeComparisons$id, attributeX$as_group)
                id.x <- paste0(unlist(lapply(str_split(attributeX$id[matchToGene], "[+]"),"[[",1)),"_" ,attributeComparisons$orf_length_bygroup_x)
                m1 <- match(id.x, paste0((orfsX$id), "_", orfsX$orf_length))
            }
            id.y <- paste0(attributeComparisons$id,"_" ,attributeComparisons$orf_length_bygroup_y)
            if(compareBy == "gene"){
                if(all(grepl("[+]", orfsY$id))){
                    orfsYid <- unlist(lapply(str_split(orfsY$id, "AS "), "[[", 2))
                }else{
                    orfsYid <- orfsY$gene_id
                }
...
orfSimilarity <- function(orfA, orfB, substitutionCost=100){
    if(is.na(orfA) | is.na(orfB)){
        ...
        if(nchar(orfA) > nchar(orfB)){
            lcs <- adist(orfA, orfB, costs=list(ins=100, del=1, sub=substitutionCost))[1,1]
        }else if(nchar(orfA) == nchar(orfB)){
            lcs <- adist(orfA, orfB, costs=list(ins=1, del=1, sub=substitutionCost))[1,1]
        }else{
            lcs <- adist(orfA, orfB, costs=list(ins=1, del=100, sub=substitutionCost))[1,1]
        }
