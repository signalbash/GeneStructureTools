...
#' @param gene_similarity compare orf to all orfs in gene?
#' @return data.frame with orf changes
#' @export
#' @import stringr
#' @examples
#' @author Beth Signal
orfDiff <- function(orfs_x, orfs_y, filter_nmd=TRUE, gene_similarity=TRUE){
    if(filter_nmd == TRUE){
    ...
    if(gene_similarity == TRUE){
        gene_matches <- lapply(orf_changes$gene_id, function(x) which(!is.na(match(orfs_x$gene_id, x))))
        id_matches <- mapply(function(x,y) rep(x,length(y)) ,(1:length(orf_changes$gene_id)), gene_matches)

        gene_matches <- unlist(gene_matches)
        id_matches  <- unlist(id_matches)
        id_matches  <- match(orf_changes$id_orf_length_y[id_matches], orfs_y$id_with_len)

        x.bygene <- as.numeric(mapply(function(x,y) orfSimilarity(x,y),
                                      orfs_x$orf_sequence[gene_matches],
                                      orfs_y$orf_sequence[id_matches]))

        orf_similarity.bygene <- data.frame(gene_id=orfs_x$gene_id[gene_matches],
                                            spliced_id=orfs_y$spliced_id[id_matches], similarity=x.bygene,
                                            length_x=orfs_x$orf_length[gene_matches], length_y=orfs_y$orf_length[id_matches])


        orf_length_kept <- orf_similarity.bygene$similarity * orf_similarity.bygene$length_x
        orf_similarity.bygene$orf_percent_new <- 1 - orf_length_kept / orf_similarity.bygene$length_y

        orf_similarity.bygene_agg <- aggregate(similarity ~ spliced_id, orf_similarity.bygene, max)
        m <- match(orf_changes$id, orf_similarity.bygene_agg$spliced_id)
        orf_changes$max_orf_kept_bygene <- orf_similarity.bygene_agg$similarity[m]

        orf_new.bygene_agg <- aggregate(orf_percent_new ~ spliced_id, orf_similarity.bygene, min)
        m <- match(orf_changes$id, orf_new.bygene_agg$spliced_id)
        orf_changes$new_orf_bygene <- orf_new.bygene_agg$orf_percent_new[m]
