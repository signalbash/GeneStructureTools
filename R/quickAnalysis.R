...
#' @param sampleTable data.frame with sample names and conditions. Only needed if filtering with counts.
...
#' jncCoords <- readjncCoordsfiles(jncFiles)
...
whippetTranscriptChangeSummary(significantEvents = whippetDiffSplice, whippetCoords,
                               eventTypes = "all",
                               gtf.exons,
                               gtf.transcripts,BSgenome = g,
                               jncCoords = jncCoords, NMD=FALSE)
...
whippetTranscriptChangeSummary <- function(significantEvents,
    ...
    if(any(eventTypes == "CE") & any(significantEvents$type == "CE")){
        ...
        orfChanges.ce <- transcriptChangeSummary(skippedExonTranscripts[skippedExonTranscripts$set=="included_exon"],
                                                 ...
                                                 g = BSgenome,NMD = NMD, whippetEvents=significantEvents.ce)
    ...
    if(any(eventTypes %in% c("AA","AD","AF","AL")) &
       any(significantEvents$type %in% c("AA","AD","AF","AL"))){
        ...
        events.significant <- events.significant[events.significant %in% events.junctions]
            ...
            jncRanges <- formatJunctions(jncCoords)
            ...
            if(nrow(significantEvents.jnc) > 0){
              ...
              orfChanges.jnc <- transcriptChangeSummary(transcriptsX = altTranscripts[altTranscripts$set==paste0(event, "_X")],
                                                      transcriptsY = altTranscripts[altTranscripts$set==paste0(event, "_Y")],
                                                       g = BSgenome,NMD = NMD, whippetEvents=significantEvents.jnc)
