...
vignette: >
  %\VignetteIndexEntry{Gene Structure Tools: Analysis and Manipulation of Spliced Gene Structures}
...
whippet_files <- list.files("~/Downloads/GeneStructureTools_VignetteFiles/",
full.names = TRUE)
...
if(!("first_last" %in% colnames(mcols(gtf.exons)))){
    ...
    gtf.exons$first_last[gtf.exons$exon_number == 
    t$Freq[match(gtf.exons$transcript_id, t$Var1)]] <- "last"
}
...
# list of files in the whippet directory
whippet_files <- list.files(system.file("extdata","whippet/", 
...
# read in psi (percent spliced in) files (default: total reads for each event)
whippet_counts <- readWhippetPSIfiles(grep(".psi", whippet_files,value = TRUE))
# rename column names to remove "example_"
colnames(whippet_counts) <- gsub("example_","",colnames(whippet_counts))

# read in diff files (events + differential splicing testing)
whippet_diff <- readWhippetDIFFfiles(grep(".diff", whippet_files,value = TRUE))
# rename comparison names to remove "example_"
whippet_diff$comparison <- gsub("example_","",whippet_diff$comparison)
...
Only the file `per_intron_results.tab`, and the .gtf file used with leafcutter needs to be read in for results processing.
```{r readLeafcutter}
# read in gtf annotation
...
significant_events <- filterSignificantWhippetEvents(
whippet_diff,
...
event.ce <- significant_events[significant_events$type=="CE",]

# we will be looking at Ndufv3 (ENSMUSG00000024038.16)
event.ce <- event.ce[event.ce$gene == "ENSMUSG00000024038.16",]
...
ranges.ce <- whippet_coords[whippet_coords$id %in% event.ce$coord]
ranges.ce
...
geneModel.all <- GeneRegionTrack(makeGeneModel(gtf.exons[gtf.exons$gene_id ==
...
event.ri <- significant_events[significant_events$type=="RI",]

# we will be looking at Srsf1 (ENSMUSG00000018379.17)
event.ri <- event.ri[event.ri$gene == "ENSMUSG00000018379.17",]

# whippet outputs the retained coordinates
ranges.ri <- whippet_coords[whippet_coords$id %in% event.ri$coord]
# need to extend whippet coords to cover the last/first base 
...
event.aa <- significant_events[significant_events$type=="AA",]

# we will be looking at Poglut1 (ENSMUSG00000034064.14)
event.aa <- event.aa[event.aa$gene == "ENSMUSG00000034064.14",]

# whippet outputs the retained coordinates
ranges.aa <- whippet_coords[whippet_coords$id %in% event.aa$coord]
...
```{r findJunctionsAA}
# find exons pairs in the gtf that bound the retained intron event
junctionPairs.aa <- findJunctionPairs(eventCoords = ranges.aa, 
                                   jncCoords = jnc_ranges, 
                                   type="AA")
junctionPairs.aa
```
...
junctionPairs.aa.manual <- GRanges(seqnames="chr16",
                            ranges=IRanges(start=c(38549433, 38549636), 
                            # splice site 1 (intronic), splice site 2 (intronic)
                                           end=c(38550083,38550083)), 
                                           # splice site 1 junction pair, 
                                           # splice site 2 junction pair
                            strand="-",
                            id=rep("AA_manual", 2))
...
altTranscripts <- replaceJunction(junctionPairs = junctionPairs.aa, 
...
event.ad <- significant_events[significant_events$type=="AD",]

# we will be looking at Mdbd3 (ENSMUSG00000035478.14)
event.ad <- event.ad[event.ad$gene == "ENSMUSG00000035478.14",]

# whippet outputs the retained coordinates
ranges.ad <- whippet_coords[whippet_coords$id %in% event.ad$coord]
...
```{r createIsoformsAD}
# find exons pairs in the gtf that bound the retained intron event
junctionPairs.ad <- findJunctionPairs(eventCoords = ranges.ad, 
                                   jncCoords = jnc_ranges, 
                                   type="AD")

# make transcripts with alternative junction usage
altTranscripts <- replaceJunction(junctionPairs = junctionPairs.ad, 
...
event.af <- significant_events[significant_events$type=="AF",]

# we will be looking at Csrp1 (ENSMUSG00000029922.15)
event.af <- event.af[event.af$gene == "ENSMUSG00000026421.14",]

# whippet outputs first (or last) exon being tested only
ranges.af <- whippet_coords[whippet_coords$id %in% event.af$coord]
...
junctionPairs.af <- findJunctionPairs(eventCoords = ranges.af, 
...
junctionPairs.af.manual <- GRanges(seqnames="chr1",
                            ranges=IRanges(start=c(135729274, 135720193), 
                            # splice site 1 (first exon), 
                            # splice site 1 (alternative first exon)
                                           end=c(135739400,135739400)), 
                            # splice site 2 (shared junction pair)  
                            strand=c("+","+"),
                            id=c("junction1", "junction2"),
                            whippet_id=rep("AF_manual", 2),
                            search=c("right", "right"), 
                            # search to the right for the shared junction pair
                            set=c("X","Y"))
...
```{r replaceJunctionAF}
# make transcripts with alternative junction usage
altTranscripts <- replaceJunction(junctionPairs = junctionPairs.af, 
...
```{r altLast}
# filter out alternative acceptor events (coded as "AF")
event.al <- significant_events[significant_events$type=="AL",]

# we will be looking at Ppm1b
event.al <- event.al[event.al$gene == "ENSMUSG00000061130.12",]

# whippet outputs first (or last) exon being tested only
ranges.al <- whippet_coords[whippet_coords$id %in% event.al$coord]
# AF/AL coordinates range are exon coordinates for the tested first/last exon
ranges.al
```
...
# find junction pairs that use the same acceptor/donor as the specified first/last exon
# i.e. find the alternative first/last exon
junctionPairs.al <- findJunctionPairs(eventCoords = ranges.al, 
                                   jncCoords = jnc_ranges, type="AL")

# make transcripts with alternative junction usage
altTranscripts <- replaceJunction(junctionPairs = junctionPairs.al, 
...
gtf.exons[gtf.exons$gene_id=="ENSMUSG00000022884.14"]),
                                    name="Reference Gene",
                                    showId=TRUE,
                                    transcriptAnnotation = "transcript")
...
# generate alternative isoforms
altIsoforms1281 <- alternativeIntronUsage(cluster, gtf.exons)
...
geneModel.ref <- GeneRegionTrack(makeGeneModel(
gtf.exons[gtf.exons$gene_id=="ENSMUSG00000034681.16"]),
                                    name="Reference Gene",
                                    showId=TRUE,
                                    transcriptAnnotation = "transcript")
...
normalTranscripts <- gtf.exons[gtf.exons$transcript_id %in% 
...
# we will be looking at Ndufv3 (ENSMUSG00000024038.16) again
event.ce <- significant_events[significant_events$gene == 
                                "ENSMUSG00000024038.16" & 
...
event.ri <- significant_events[significant_events$gene == 
                                            "ENSMUSG00000018379.17" 
