...
author: "Beth Signal"
date: "18 January 2017 / Version 0.99.0"
output:
    html_document:
        toc: true
        theme: united
...
leafcutter_files <- list.files("~/Downloads/GeneStructureTools_VignetteFiles/leafcutter/",full.names = TRUE)
...
###leafcutter
```{r leafcutterSummary}
intron_results_significant <- intron_results[intron_results$p.adjust < 0.0001,]

leafcutter_summary <- leafcutterTranscriptChangeSummary(intron_results_significant, 
                                                        gtf.exons = gtf.exons,
                                                        BSgenome = g,
                                                        NMD = FALSE,
...
altIsoforms1396_dnreg <- altIsoforms1396[grep("dnre", altIsoforms1396$transcript_id)]
# upregulated isoforms
altIsoforms1396_upreg <- altIsoforms1396[grep("upre", altIsoforms1396$transcript_id)]

# visualise
gtr <- Gviz::GenomeAxisTrack()

geneModel.ref <- GeneRegionTrack(makeGeneModel(gtf.exons[gtf.exons$gene_id=="ENSMUSG00000022884.14"]),
                                    name="Reference Gene",
                                    showId=TRUE,transcriptAnnotation = "transcript")
...
plotTracks(list(geneModel.ref,geneModel.dnreg, geneModel.upreg), extend.left = 1000, extend.right = 1000)
...
# downregulated isoforms
altIsoforms1281_dnreg <- altIsoforms1281[grep("dnre", altIsoforms1281$transcript_id)]
# upregulated isoforms
altIsoforms1281_upreg <- altIsoforms1281[grep("upre", altIsoforms1281$transcript_id)]

# visualise
gtr <- Gviz::GenomeAxisTrack()

geneModel.ref <- GeneRegionTrack(makeGeneModel(gtf.exons[gtf.exons$gene_id=="ENSMUSG00000034681.16"]),
                                    name="Reference Gene",
                                    showId=TRUE,transcriptAnnotation = "transcript")
